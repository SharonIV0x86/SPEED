...
TEST_F(AccessRegistryTest, AddProcessFailsIfNotInGlobalRegistry) {
    AccessRegistry reg(tempDir, procName);
    testing::internal::CaptureStdout();
    reg.addProcessToList("UnknownProc");
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("[ERROR]: Process cannot be added"), std::string::npos);
    EXPECT_FALSE(reg.checkAccess("UnknownProc"));
}
...
TEST_F(AccessRegistryTest, RemoveAccessFileHandlesMissingFileGracefully) {
    ...
    reg.removeAccessFile();
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("[ERROR]: Unable to remove the access file"), std::string::npos);
}
