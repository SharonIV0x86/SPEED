...
TEST_F(BinaryManagerTest, WriteBinaryEmptyPayload) {
    ...
    fs::path writtenFile;
    for (auto &entry : fs::directory_iterator(tempDir / procName))
        if (entry.path().extension() == ".ospeed") writtenFile = entry.path();
...
TEST_F(BinaryManagerTest, ReadBinaryInvalidPathThrows) {
    EXPECT_THROW(BinaryManager::readBinary(tempDir / "nonexistent.ospeed"), std::runtime_error);
}
...
TEST_F(BinaryManagerTest, WriteBinaryFileExists) {
    ...
    bool firstWrite = BinaryManager::writeBinary(msg, tempDir, seqNumber, procName);
    EXPECT_TRUE(firstWrite);

    bool secondWrite = BinaryManager::writeBinary(msg, tempDir, seqNumber, procName);
...
TEST_F(BinaryManagerTest, NonexistentDirectoryFailsGracefully) {
    ...
    bool success = BinaryManager::writeBinary(msg, invalidDir, seqNumber, procName);
